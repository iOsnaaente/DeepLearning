...
def compute_fitness_individual( individual, y_hat ):
...
class Thredis ( Thread ):
  def __init__(self, function, param, hat):
  ...
  def run (self):
    self.result = self.f ( self.param, self.hat )
...
def genalg(w, h, y_hat, pop_size, elite_size, epochs, mutation_rate):
  ...
  for generation in range( epochs + 1 ):
    ...
    for ind in population:
      t = Thredis( function = compute_fitness_individual, param = ind, hat = y_hat )
      t.start()
      threads.append( t )
    
    for n,t in enumerate(threads):
      t.join()
      population[n] = t.result
    ...
    if generation % 100 == 0:
      ...
      plt.imshow(img_gen)
      plt.savefig('D:\Desktop\git\Algoritmo genético/Generation%sFit%2.10f.png'%(generation, fit), dpi =600)
...
images, x, y = genalg( W, H, y_hat, 150, 1, 10, 0.0025)
